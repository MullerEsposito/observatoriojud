...
def query_dou_history(start_date="2019-01-01", end_date="2024-12-31", use_cache=True):
  """
  Query DOU para eventos do Poder Judiciário com TI (nomeações, vacâncias, etc.)
  """
  cache_name = f"dou_historical_jud_{start_date.replace('-', '')}_{end_date.replace('-', '')}.parquet"
  ...
  query = f"""
    ...
    WHERE data_publicacao BETWEEN '{start_date}' AND '{end_date}'
      -- 1. Precisa ser do Judiciário
      AND LOWER(orgao) LIKE '%poder judiciário%'
      -- 2. Precisa ser movimentação funcional
      AND (
        LOWER(texto_completo) LIKE '%nomea%'
        OR LOWER(texto_completo) LIKE '%posse%'
        OR LOWER(texto_completo) LIKE '%vacância%'
        OR LOWER(texto_completo) LIKE '%vago%'
      ...
      AND (
        ...
        OR LOWER(texto_completo) LIKE '%analista judiciário%'
        OR LOWER(texto_completo) LIKE '%técnico judiciário%'
        OR LOWER(texto_completo) LIKE '%apoio especializado%'
        OR LOWER(texto_completo) LIKE '%análise de sistemas%'
        OR LOWER(texto_completo) LIKE '%desenvolvimento de sistemas%'
        OR LOWER(texto_completo) LIKE '%infraestrutura%'
        OR LOWER(texto_completo) LIKE '%rede%'
        OR LOWER(texto_completo) LIKE '%suporte técnico%'
        OR LOWER(texto_completo) LIKE '%segurança da informação%'
        OR LOWER(texto_completo) LIKE '%banco de dados%'
        OR LOWER(texto_completo) LIKE '%cibersegurança%'
        OR LOWER(texto_completo) LIKE '%pje%'
      )
  """
